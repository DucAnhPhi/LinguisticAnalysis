...
class DatasetTests(unittest.TestCase):
    ...
    def test_normalize(self):
        m = np.array([
            [1, 4, 0.5, 9], [0, 2, 0.2, 2], [0, 1, 0.01, 8], [1, 2.5, 0.3, 3]
        ])
        norm = np.array([
            [1, 1, 1, 1], [0, 0.5, 0.4, 0.222],
    ...
    def test_combined_keywords(self):
        ...
        keywords = {
            "fake": 0, "media": 0, "news": 0,
            "women": 0, "hillary": 0, "trump": 0
        }
        self.assertEqual(ds.get_combined_keywords(t1, t2), keywords)

    def test_keywords_count(self):
        t =  [
            ["make", "america", "great", "again"],["america", "was", "great"]
        ]
        dictionary = {
            "make": 0, "america": 0, "great": 0, "again": 0, "was": 0
        }
